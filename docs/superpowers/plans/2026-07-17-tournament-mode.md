...
- [ ] **Step 1: 실패하는 테스트 작성** — `tests/test_model_select.py`
...
- [ ] **Step 1: 실패하는 테스트 추가** — `tests/test_debate_engine.py` 끝에
...
- [ ] **Step 2: 실패하는 테스트 작성** — `tests/test_tournament.py`
...
- [ ] **Step 1: 실패하는 테스트 추가** — `tests/test_pdf_export.py` 끝에
...
- [ ] **Step 3: pdf_export.py에 구현** (기존 `_mc`, `_find_font`, 폰트 로딩 재사용)
...
- [ ] **Step 1: 사이드바 — 모델 선택 + 종목 다중 입력 + 고급 설정**
