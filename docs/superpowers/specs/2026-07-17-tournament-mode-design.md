...
- [ ] 단일 종목 입력 시 기존 동작과 완전 동일 (회귀 없음, 기존 테스트 통과)
- [ ] tournament() 단위 테스트: mock ask_fn으로 2종목 흐름(개별→변론→랭킹), 1종목 실패 시 스킵 동작
- [ ] 모델 선택: 선택 모델이 CLI/API 호출에 실제 반영되는지 (mock으로 cmd/model 인자 검증)
- [ ] 타임아웃 슬라이더 값이 debate()에 전달되는지
- [ ] 랭킹 JSON 파싱 실패 시 재시도 및 raw 반환
- [ ] 토너먼트 PDF 한글 생성
- [ ] PDF 분량 옵션: mock ask_fn으로 목표 글자수가 프롬프트에 반영되는지, 요약 PDF 바이트 생성 확인
- [ ] 실전 검수: 2종목×1라운드 실호출 1회 (전체 흐름 end-to-end)
- [ ] README 갱신 + GitHub push
